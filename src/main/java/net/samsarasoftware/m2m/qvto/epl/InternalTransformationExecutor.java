...
public  class InternalTransformationExecutor {
	...
	protected UnitProxy getUnit(URI fURI) {
			if(resolver==null){
				return UnitResolverFactory.Registry.INSTANCE.getUnit(fURI);
			}else{
				return getResolver(fURI.trimSegments(1)).resolveUnit(getQualifiedName(fURI));
			}
	}
	...
	public  class TracesAwareExecutor extends InternalTransformationExecutor {
